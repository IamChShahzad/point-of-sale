...
c.execute("""CREATE TABLE IF NOT EXISTS items (
                ...
                uom TEXT CHECK (uom IN ('each', 'oz', 'lb', 'kg', 'gram', 'liter', 'gallon', 'cup')),
                size VARCHAR(32),
...
def addData(item_name, barcode_number, department, cost_per_piece, sales_price,
              quantity, tax, deposit, upc, brand, description, uom, size, ebt_eligible,
              weight_item, age_restricted, case_upc, case_qty, case_cost, cash_price,
              card_price, markup):
    params = (item_name, barcode_number, department, cost_per_piece, sales_price,
              quantity, tax, deposit, upc, brand, description, uom, size, ebt_eligible,
              weight_item, age_restricted, case_upc, case_qty, case_cost, cash_price,
              card_price, markup)
    try:
...
def getPurchasePrices():
    c.execute("SELECT cost_per_piece FROM items")
    rows = c.fetchall()
    return list(rows)
...
def updatePurchasePrice(barcode_number,new_pp):
    ...
    try:
        c.execute("UPDATE items SET cost_per_piece = (?) WHERE barcode = (?)", params)
        conn.commit()
    except Exception:
        pass
...
def getPPFromBarcode(barcode_number):
    param = (barcode_number,)
    try:
        pp = c.execute("SELECT cost_per_piece FROM items WHERE barcode = (?)", param)
        final_pp = pp.fetchone()
