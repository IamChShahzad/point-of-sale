...
def endShift(*args):
    ...
    #  Perform batch settlement before saving and printing
    try:
        result = settle_batch_out(
            tpn=TPN,
            auth_key=AUTH_KEY
        )
        if result.get("status") == "Approved" or result.get("message", {}).get("GeneralResponse", {}).get("StatusCode") == "0":
            shiftReport += "\n\n✅ Batch Settlement Successful"
        else:
            shiftReport += "\n\n⚠️ Settlement Failed: " + str(result.get("message"))
    except Exception as e:
        shiftReport += f"\n\n❌ Batch Settlement Error: {str(e)}"
