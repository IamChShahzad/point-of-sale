...
class RetailScreen(MDScreen):
    ...
    def process_refund(self, total_refund):
        self.dialog.dismiss()

        # from kivyScripts.payment_gateway import refund_sale
        response = refund_sale(
            tpn=TPN,
            auth_key=AUTH_KEY,
            amount=total_refund
        )

        if response.get("success") and response.get("status") == "Approved":
            MDDialog(title=f"✅ Refund Approved\nAmount: ${total_refund}").open()
        else:
            MDDialog(title="❌ Refund Failed", text=str(response.get("message"))).open()

    def process_void(self, total_refund):
        self.dialog.dismiss()

        # Full transaction total (original)
        total = round(self.cart["line_total"].sum(), 2)
        reference_id = getattr(self, "last_reference_id", None)

        if not reference_id:
            MDDialog(title="❌ Error", text="No Reference ID found for void.").open()
            return

        if abs(total_refund - total) > 0.01:
            MDDialog(title="❌ Cannot Void", text="To void, refund must equal full transaction amount.").open()
            return
        ...
        if response.get("success") and response.get("status") == "Approved":
            MDDialog(title="✅ Transaction Voided").open()
        else:
            MDDialog(title="❌ Void Failed", text=str(response.get("message"))).open()
    ...
    def update_table(self, *args):
        ...
        self.sv = ScrollView(
            size_hint=[None, None],
            size=(Window.width * 0.5, Window.height * 0.665),
            pos_hint={'x': 0.125, 'y': 0.235},
            bar_width='8px',
            scroll_type=["bars", "content"],
            scroll_distance=25
        )

        datatable = MDGridLayout(
            cols=9,  # Added one more column for EBT
        ...
        # Add EBT column to headers
        columns = ["Description", "EBT", "Qty", "Price", "Disc./Coupon\nAmount", "Tax\nAmount", "Deposit\nAmount", "Line Total"]
        ...
        # Track EBT subtotal
        self.ebt_subtotal = 0.0

        for row_num, i in enumerate(self.get_rows()):
            item_name = i[0]
            is_ebt = False
            
            # Check if item is EBT eligible
            if hasattr(self, 'cart') and not self.cart.empty:
                try:
                    # Get department for the current row
                    department = self.cart.iloc[row_num].get("department", "")
                    if department.lower() == "ebt":
                        is_ebt = True
                        self.ebt_subtotal += float(i[-1])  # Line total
                except Exception:
                    pass

            # Add EBT tag to name if applicable
            if is_ebt:
                item_name += " [EBT]"

            datatable.add_widget(MDLabel(
                text=item_name,
                size_hint_x=None,
                width=Window.width * 0.27,
                markup=True,
                halign="left",
                md_bg_color=[0.204, 0.204, 0.204, 0.15]
            ))

            for col_num, j in enumerate(i[1:-1]):
                datatable.add_widget(BaseButton(
                    text=str(j),
                    anchor_x="center",
                    size_hint=[1, 1],
                    font_size=sp(24),
                    md_bg_color=[0.204, 0.204, 0.204, 0.15],
                    on_press=partial(self.on_row_press, row_num, columns[col_num + 1])
                ))
            ...
            datatable.add_widget(MDRaisedButton(
                text="Delete",
                md_bg_color=[0.256, 0, 0, 0.5],
                on_press=partial(self.del_row, row_num),
                size_hint=[1, 1]
            ))
    ...
    def mainButton(self,widget,*args):
        ...
        if button == "[b]Delete\nTransaction[/b]":
            ...
            total_refund = round(abs(self.cart["line_total"].sum()), 2)

            if total_refund <= 0:
                MDDialog(title="❌ Error", text="Refund amount must be greater than 0.").open()
                return

            self.dialog = MDDialog(
                title="Choose Return Method",
                text=f"Refund Amount: ${total_refund}\n\nRefund is for specific items.\nVoid cancels entire transaction.",
                buttons=[
                    MDFlatButton(
                        text="Refund",
                        on_release=lambda x: self.process_refund(total_refund)
                    ),
                    MDFlatButton(
                        text="Void Transaction",
                        on_release=lambda x: self.process_void(total_refund)  # Pass total for validation
                    ),
                ]
            )
            self.dialog.open()
    ...
    def endTransaction(self,value,*args,**kwargs):
        tax=round(self.cart["tax_value"].sum(),2)
        total = round(self.cart["line_total"].sum(),2)
        ebt_subtotal = round(self.cart[self.cart["department"].str.lower() == "ebt"]["line_total"].sum(), 2)


        if len(self.cart):
            if value == "Credit/Debit":
                card_per = round((total * CARD_PERCENTAGE) / 100,2) 
                card_tax = card_per + total
                # print("CARD TAX:", card_tax)
                response = credit_sale(tpn=TPN, auth_key=AUTH_KEY, amount=card_tax)
                print("EBT CASH PAYMENT RESPONSE:", response)
                message = response.get("message", {})
                if isinstance(message, dict):
                    self.last_reference_id = message.get("ReferenceId", "")
                else:
                    self.last_reference_id = ""

                if response.get("success") and response.get("status") == "Approved":
                    self.saveTransactions(total, tax, self.cart.copy(deep=True), "Credit")
                    self.cart.drop(self.cart.index, inplace=True)
                    self.update_table()
                    MDDialog(title=f"✅ Credit Payment Approved\nAmount: ${round(total, 2)}\nEBT Subtotal: ${ebt_subtotal}").open()
                else:
                    MDDialog(title="❌ Credit Payment Failed", text=str(response.get("message"))).open()

            elif value == "EBT":
                response = ebt_cash(tpn=TPN, auth_key=AUTH_KEY, amount=total)
                # response = ebt_cash(tpn="248925801908", auth_key="Z8Ed2sfg98", amount=total)
                print("EBT CASH PAYMENT RESPONSE:", response)
                message = response.get("message", {})
                if isinstance(message, dict):
                    self.last_reference_id = message.get("ReferenceId", "")
                ...
                if response.get("success") and response.get("status") == "Approved":
                    self.saveTransactions(total, tax, self.cart.copy(deep=True), "EBT_Cash")
                    self.cart.drop(self.cart.index, inplace=True)
                    self.update_table()
                    MDDialog(title=f"✅ EBT Cash Payment Approved\nAmount: ${round(total, 2)}\nEBT Subtotal: ${ebt_subtotal}").open()
                else:
                    MDDialog(title="❌ EBT Cash Payment Failed", text=str(response.get("message"))).open()

            elif value == "Cash":
                self.dialog.dismiss(force=True)
                try:
                    # Get cash input from user
                    user_cash_input = self.dialog.content_cls.ids.textField.text
                    value = float(user_cash_input)
                except:
                    value = None

                self.last_reference_id = str(uuid.uuid4().hex[:12])
                ...
                if isinstance(value, int) or isinstance(value, float):
                    if value >= total:
                        ...
                        self.dialog = MDDialog(
                            title=f"""\n Sub-Total  :      {round(total - tax, 2)}\nEBT Subtotal: {ebt_subtotal}\n{'-'*int(Window.width * 0.0265)}\n[b]{' '*5}TOTAL   :      {total}\n\n{' '*7}CASH   :      {value}\nCHANGE : {change}[/b]""",
                            size=[Window.width * 0.4, Window.height * 0.7],
                            size_hint=[None, None],
                    )
                        self.dialog.open()
                    else:
                        self.dialog = MDDialog(
                            title="❌ ERROR\n\nCash given is less than total. Please accept the full cash amount.",
