...
class InventoryScreen(MDScreen):
    ...
    nameField = None
    dialog = None
    case_qty, case_cost, cost_per_piece, markup, price = None, None, None, None, None
    ...
    def on_pre_leave(self, *args):
        ...
        if not self.dialog is None:
            ...
            self.dialog = None
        if not self.case_qty is None:
            self.case_qty = None
        if not self.case_cost is None:
            self.case_cost = None
        if not self.cost_per_piece is None:
            self.cost_per_piece = None
        if not self.markup is None:
            self.markup = None
    ...
    def getLabel(self, y, *args):
        x = {"add": "Add New Inventory", "new": "New Product", "update": "Update Product", "delete": "Delete Product",
             "look": "Look-Up Product", "department": "Department Categories", "tax_deposit": "Tax and Deposit Categories"}
    ...
    def newInventory(self, *args):
        ...
        layout = MDRelativeLayout(
            pos_hint={"x": 0, "y": 0.15}, size_hint=[1, 0.7],)
        ...
        layout.add_widget(
            MDLabel(text="[b] Enter Barcode : [/b]",
                    pos_hint={"x": 0.02, "y": 0.915}, size_hint=[0.35, 0.05], markup=True)
        )
        self.layout = layout
    ...
    def newInventoryEnter(self, *args):
        ...
        if inv.isBarcode(barcode):
            ...
            columns = ["Name", "Barcode", "Department Category", "Cost Per Piece",
                       "Price", "Quantity", "Tax Category", "Deposit Category"]
            columns = dict(
                zip(columns, inv.getItemFromBarcode(self.barcode.text.strip())))
            grid = MDGridLayout(cols=4, pos_hint={"x": 0.04, "y": 0.24}, size_hint=[
                                0.9, 0.4], spacing=[70, 20])
            for i in ["Barcode", "Name", "Department Category", "Price", "Tax Category", "Cost Per Piece", "Deposit Category", "Quantity"]:
                j = columns[i]
                grid.add_widget(MDLabel(text=f"[b]{i} : [/b]", markup=True))
                grid.add_widget(
                    MDLabel(text=f"{j}", markup=True, halign="right"))
    ...
    def newProduct(self, *args):
        def isRequired(field):
            optional_fields = ["Brand", "Description", "Size", "Case UPC", "Case QTY",
                               "Case Cost", "Cost Per Piece", "Markup"]
        ...
        def onValidate(*args):
            field = args[0]
            instance = args[1]
            if not instance.focus and instance.text.strip() != "":
                if field == "Cash Price":
                    try:
                        cash_price = float(instance.text.strip())
                        instance.text = str(cash_price)
                    except Exception as e:
                        if self.dialog:
                            self.remove_widget(self.dialog)
                        self.dialog = MDDialog(
                            title="!! Invalid Cash Price !!",
                            text="Enter a decimal number."
                        )
                        self.dialog.open()
                        return
                    columnData["Card Price"].text = str(cash_price)

                if field == "Case QTY" or field == "Case Cost":
                    if field == "Case QTY":
                        try:
                            self.case_qty = int(instance.text.strip())
                            instance.text = str(self.case_qty)
                        except Exception as e:
                            if self.dialog:
                                self.remove_widget(self.dialog)
                            self.dialog = MDDialog(
                                title="!! Invalid Case QTY !!",
                                text="Enter a interger number."
                            )
                            self.dialog.open()
                            return
                    if field == "Case Cost":
                        try:
                            self.case_cost = float(instance.text.strip())
                            instance.text = str(self.case_cost)
                        except Exception as e:
                            if self.dialog:
                                self.remove_widget(self.dialog)
                            self.dialog = MDDialog(
                    ...
                    if self.case_qty and self.case_cost:
                        cost_per_piece = self.case_cost/self.case_qty
                        columnData["Cost Per Piece"].text = str(cost_per_piece)
                        self.cost_per_piece = cost_per_piece

                if field == "Cost Per Piece" or field == "Markup":
                    if field == "Cost Per Piece":
                        try:
                            self.cost_per_piece = float(instance.text.strip())
                            instance.text = str(self.cost_per_piece)
                        except Exception as e:
                            if self.dialog:
                                self.remove_widget(self.dialog)
                            self.dialog = MDDialog(
                                title="!! Invalid Cost Per Piece !!",
                                text="Enter a decimal number."
                            )
                            self.dialog.open()
                            return
                    if field == "Markup":
                        try:
                            self.markup = float(instance.text.strip())
                            instance.text = str(self.markup)
                        except Exception as e:
                            if self.dialog:
                                self.remove_widget(self.dialog)
                            self.dialog = MDDialog(
                                title="!! Invalid Markup !!",
                                text="Enter a percentage value."
                            )
                            self.dialog.open()
                            return
                    
                    if self.cost_per_piece and self.markup:
                        price = self.cost_per_piece + (self.cost_per_piece * self.markup / 100)
                        columnData["Price"].text = str(price)
                        self.price = price
        ...
        columns = ["Barcode", "Name", "Department Category", "Cost Per Piece",
                   "Price", "Quantity", "Tax Category", "Deposit Category",
                   "UPC", "Brand", "Description", "UOM", "Size", "EBT Eligible",
                   "Weight Item", "Age Restricted", "Case UPC", "Case QTY",
                   "Case Cost", "Cash Price", "Card Price", "Markup"]
        columnData = {}

        for i in columns:
            if i not in ["Department Category", "Tax Category", "Deposit Category",
                         "UOM", "EBT Eligible", "Weight Item", "Age Restricted"]:
                stack.add_widget(MDLabel(
                    text=f"[b]{i} : [/b]", font_size=fontsize['label'], markup=True, valign="center"))
                columnData[i] = MDTextField(
                    hint_text=f"Enter {i} Here... ", mode="rectangle", required=isRequired(i), on_text_validate=partial(onValidate, i))
                columnData[i].bind(
                        focus=partial(onValidate, i),
                    )
                stack.add_widget(columnData[i])
            elif i in ["EBT Eligible", "Weight Item", "Age Restricted"]:
                stack.add_widget(MDLabel(
                    text=f"[b]{i} : [/b]", font_size=fontsize['label'], markup=True, valign="center"))

                inner_stack = MDGridLayout(size_hint=(1, None), cols=3, adaptive_height=True,
                                           row_force_default=True, row_default_height=Window.height/11,
                                           col_force_default=True, col_default_width=Window.width/8,
                                           padding=[0, 70, 50, 0], spacing=[0, 0])

                inner_stack.add_widget(MDLabel(
                    text=f"No", font_size=fontsize['label'], markup=True, valign="center"))

                columnData[i] = MDSwitch(pos_hint={"center_x": 0.5})

                inner_stack.add_widget(columnData[i])

                inner_stack.add_widget(MDLabel(
                    text=f"Yes", font_size=fontsize['label'], markup=True, valign="center"))

                stack.add_widget(inner_stack)

            elif i == "Department Category":
                department_category_dropdown = DropDown()
                for idx in inv.getCategory("Department_Category"):
                    btn1 = Button(text=f"{idx[2]}\n{idx[3]}", size_hint=[
                                 1, None], halign="left", background_color=[0.150, 0.60, 1.0, 1.0])
                    btn1.bind(on_release=lambda btn1: department_category_dropdown.select(btn1.text))
                    department_category_dropdown.add_widget(btn1)

                department_category_btn = Button(text='Select Category', size_hint=[1, 1])
                department_category_btn.bind(on_release=department_category_dropdown.open)
                department_category_dropdown.bind(on_select=lambda instance,
                               x: setattr(department_category_btn, 'text', x))

                stack.add_widget(
                    MDLabel(text=f"[b]{i} : [/b]", font_size=fontsize['label'], markup=True))
                columnData[i] = department_category_btn
                stack.add_widget(columnData[i])
            elif i == "UOM":
                uom_dropdown = DropDown()
                for idx in ["each", "oz", "lb", "kg", "gram", "liter", "gallon", "cup"]:
                    btn2 = Button(text=f"{idx}", size_hint=[
                                 1, None], halign="left", background_color=[0.150, 0.60, 1.0, 1.0])
                    btn2.bind(on_release=lambda btn2: uom_dropdown.select(btn2.text))
                    uom_dropdown.add_widget(btn2)

                uom_btn = Button(text='Select UOM', size_hint=[1, 1])
                uom_btn.bind(on_release=uom_dropdown.open)
                uom_dropdown.bind(on_select=lambda instance,
                               x: setattr(uom_btn, 'text', x))

                stack.add_widget(
                    MDLabel(text=f"[b]{i} : [/b]", font_size=fontsize['label'], markup=True))
                columnData[i] = uom_btn
                stack.add_widget(columnData[i])
            elif i == "Tax Category":
                dropdown2 = DropDown()
                for idx in inv.getCategory("Tax_Category"):
                    btn3 = Button(text=f"{idx[2]}\n{idx[3]}", size_hint=[
                                 1, None], halign="left", background_color=[0.150, 0.60, 1.0, 1.0])
                ...
                dropdown2.bind(on_select=lambda instance,
                               x: setattr(mainbutton2, 'text', x))
                ...
                for idx in inv.getCategory("Deposit_Category"):
                    btn4 = Button(text=f"{idx[2]}\n{idx[3]}", size_hint=[
                                 1, None], background_color=[0.150, 0.60, 1.0, 1.0])
                    btn4.bind(on_release=lambda btn4: dropdown3.select(btn4.text))
                    dropdown3.add_widget(btn4)

                mainbutton3 = Button(text='Select Category', size_hint=[1, 1])
                mainbutton3.bind(on_release=dropdown3.open)
                dropdown3.bind(on_select=lambda instance,
                               x: setattr(mainbutton3, 'text', x))
    ...
    def newProductConfirm(self, columnData, *args, **kwargs):
        error = {}
        for i in columnData:
            if i in ["Cost Per Piece", "Brand", "Description", "UOM", "Size", "EBT Eligible", "Weight Item", 
                     "Age Restricted", "Case UPC", "Case QTY", "Case Cost", "Markup"]:
                continue

            if columnData[i].text.strip() == "" or columnData[i].text.strip() == "Select Category" or columnData[i].text.strip() == "Select UOM":
                error[i] = True
            else:
                error[i] = False
        if any(error.values()):
            ...
            self.dialog.open()
        elif not re.search(r"^\d*$", columnData['Cost Per Piece'].text.strip().replace(".", "")):
            if self.dialog:
                self.remove_widget(self.dialog)
            self.dialog = MDDialog(
                title="!! Invalid Input, Cost Per Piece !!\nMust be Number or Decimal value only...",
            )
            self.dialog.open()
        elif (not re.search(r"^\d*$", columnData['Price'].text.strip().replace(".", ""))) or float(columnData['Price'].text.strip()) <= 0:
            if self.dialog:
                self.remove_widget(self.dialog)
            self.dialog = MDDialog(
            ...
            inv.addData(columnData["Name"].text.strip(), columnData["Barcode"].text.strip(),
                ...
                "\n")[0],
                columnData["Cost Per Piece"].text.strip() if columnData["Cost Per Piece"].text.strip() != "" else None,
                columnData["Price"].text.strip(), columnData["Quantity"].text.strip(),
                columnData["Tax Category"].text.strip().split("\n")[0], columnData["Deposit Category"].text.strip().split("\n")[0],
                columnData["UPC"].text.strip(),
                columnData["Brand"].text.strip() if columnData["Brand"].text.strip() != "" else None,
                columnData["Description"].text.strip() if columnData["Description"].text.strip() != "" else None,
                columnData["UOM"].text.strip() if columnData["UOM"].text.strip() != "Select UOM" else None,
                columnData["Size"].text.strip() if columnData["Size"].text.strip() != "" else None,
                int(columnData["EBT Eligible"].active),
                int(columnData["Weight Item"].active), int(columnData["Age Restricted"].active),
                columnData["Case UPC"].text.strip() if columnData["Case UPC"].text.strip() != "" else None,
                columnData["Case QTY"].text.strip() if columnData["Case QTY"].text.strip() != "" else None,
                columnData["Case Cost"].text.strip() if columnData["Case Cost"].text.strip() != "" else None,
                columnData["Cash Price"].text.strip(), columnData["Card Price"].text.strip(),
                columnData["Markup"].text.strip() if columnData["Markup"].text.strip() != "" else None
                )
            if self.dialog:
                self.remove_widget(self.dialog)
            self.dialog = MDDialog(
                title="!! Item was successfully added !!",
            )
            self.dialog.open()
            self.newProduct()
    ...
    def updateProductBarcode(self, *args):
        if inv.isBarcode(self.barcode.text.strip()):
            ...
            stack.add_widget(
                MDLabel(text="", font_size=fontsize['label'], markup=True, valign="center"))
            stack.add_widget(MDLabel(text="[b][u]Current Value[/u][/b]",
                             font_size=fontsize['label'], markup=True, halign="center"))
            stack.add_widget(MDLabel(
                text="[b][u]Update Value[/u][/b]", font_size=fontsize['label'], markup=True, halign="center"))
            stack.add_widget(
                MDLabel(text="", font_size=fontsize['label'], markup=True, valign="center"))
            ...
            for i, j in zip(columns, inv.getItemFromBarcode(self.barcode.text.strip())):
                ...
                stack.add_widget(MDLabel(
                    text=f"[b]{i} : [/b]", font_size=fontsize['label'], markup=True, valign="center"))
                stack.add_widget(
                    MDLabel(text=f"{j}", font_size=fontsize['label'], markup=True, valign="center"))
                if i not in ["Department Category", "Tax Category", "Deposit Category"]:
                    columnData[i] = MDTextField(
                        hint_text=f"Enter {i} Here... ", mode="rectangle")
                    if i in ["Cost Per Piece", "Price", "Quantity"]:
                        columnData[i].input_filter = "float"
                    stack.add_widget(columnData[i])
                elif i == "Department Category":
                    dropdown1 = DropDown()
                    for idx in inv.getCategory("Department_Category"):
                        btn6 = Button(text=f"{idx[2]}\n{idx[3]}", size_hint=[
                                     1, None], halign="left", background_color=[0.150, 0.60, 1.0, 1.0])
                        btn6.bind(
                            on_release=lambda btn6: dropdown1.select(btn6.text))
                        dropdown1.add_widget(btn6)
                    mainbutton1 = Button(
                        text='Select Category', size_hint=[1, 1])
                    mainbutton1.bind(on_release=dropdown1.open)
                    dropdown1.bind(on_select=lambda instance,
                                   x: setattr(mainbutton1, 'text', x))
                    columnData[i] = mainbutton1
                    stack.add_widget(columnData[i])
                elif i == "Tax Category":
                    dropdown2 = DropDown()
                    for idx in inv.getCategory("Tax_Category"):
                        btn5 = Button(text=f"{idx[2]}\n{idx[3]}", size_hint=[
                                     1, None], halign="left", background_color=[0.150, 0.60, 1.0, 1.0])
                        btn5.bind(
                            on_release=lambda btn5: dropdown2.select(btn5.text))
                    ...
                    dropdown3 = DropDown()
                    for idx in inv.getCategory("Deposit_Category"):
                        btn7 = Button(text=f"{idx[2]}\n{idx[3]}", size_hint=[
                                     1, None], background_color=[0.150, 0.60, 1.0, 1.0])
                        btn7.bind(
                            on_release=lambda btn7: dropdown3.select(btn7.text))
                        dropdown3.add_widget(btn7)
                    mainbutton3 = Button(
                        text='Select Category', size_hint=[1, 1])
                    mainbutton3.bind(on_release=dropdown3.open)
                    dropdown3.bind(on_select=lambda instance,
                ...
                stack.add_widget(
                    MDRaisedButton(text='Update', size_hint=[1, 1], on_press=partial(
                        self.updateValue, i, columnData[i], self.barcode.text.strip()))
                )
            self.sv.add_widget(stack)
            self.layout.add_widget(self.sv)
        else:
    ...
    def updateConfirm(self, col, value, barcode, *args):
        ...
        if col == "Name":
            inv.updateName(barcode, value),
        elif col == "Department Category":
            inv.updateDepartment(barcode, value.split("\n")[0]),
        elif col == "Cost Per Piece":
            inv.updatePurchasePrice(barcode, value),
        elif col == "Price":
            inv.updateSellPrice(barcode, value),
        elif col == "Quantity":
            inv.updateQuantity(barcode, value),
        elif col == "Tax Category":
            inv.updateTax(barcode, value.split("\n")[0]),
        elif col == "Deposit Category":
            inv.updateDeposit(barcode, value.split("\n")[0]),
    ...
    def deleteProduct(self, *args):
        ...
        self.chnageBack()
        layout = MDRelativeLayout(
            pos_hint={"x": 0, "y": 0.15}, size_hint=[1, 0.6],)
        self.barcode = MDTextField(hint_text="Enter or Scan Barcode Here... ", mode="fill", size_hint=[0.35, 0.05],
    ...
    def deleteEnter(self, *args):
        if not self.sv is None:
            self.layout.remove_widget(self.sv)
            self.sv = None
        self.sv = MDRelativeLayout(
            pos_hint={"x": 0.45, "y": 0}, size_hint=[0.5, 0.7])
        if self.barcode.text.strip() != "" and inv.isBarcode(self.barcode.text.strip()):
            self.sv.add_widget(self.productDisplay(
                inv.getItemFromBarcode(self.barcode.text.strip())))
            self.layout.add_widget(MDRaisedButton(text="Delete", size_hint=[
                                   0.3, 0.2], on_press=self.deleteDialog, pos_hint={"x": 0.045, "y": 0.27}, md_bg_color=[0.255, 0, 0, 0.5]))
        else:
        ...
        if inv.getNameFromBarcode(self.barcode.text.strip()):
            ...
            self.dialog = MDDialog(
                ...
                buttons=[MDRaisedButton(
                    text="Delete", on_press=self.deleteConfirm, font_size=fontsize['button'])],
            )
            self.dialog.open()
    ...
    def deleteConfirm(self, *args):
        ...
        if not self.sv is None:
            ...
            self.sv = None
        self.sv = MDRelativeLayout(
            pos_hint={"x": 0.45, "y": 0.15}, size_hint=[0.4, 0.7])
        if self.barcode.text.strip() != "" and inv.isBarcode(self.barcode.text.strip()):
    ...
    def lookUpProduct(self, *args):
        ...
        self.layout = MDRelativeLayout(
            pos_hint={"x": 0, "y": 0.15}, size_hint=[1, 0.6],)
        self.layout.add_widget(
            MDLabel(text="Enter Product BARCODE ; ", pos_hint={
                    "x": 0.03, "y": 0.97}, size_hint=[0.35, 0.05])
        )
        self.layout.add_widget(
            MDLabel(
                text="[b] OR [/b]", pos_hint={"x": 0.03, "y": 0.6}, size_hint=[0.35, 0.05], markup=True)
        )
        self.layout.add_widget(
            MDLabel(text="Enter Product NAME ; ", pos_hint={
                    "x": 0.03, "y": 0.42}, size_hint=[0.35, 0.05])
        )
        self.barcode = MDTextField(hint_text="Enter or Scan Barcode Here... ", mode="fill", size_hint=[0.35, 0.05],
    ...
    def lookup(self, *args):
        ...
        if self.barcode.text.strip() != "" and inv.isBarcode(self.barcode.text.strip()):
            stack.add_widget(MDLabel(
                text=f"[b]Barcode : {self.barcode.text.strip()}[/b]", markup=True, size_hint_y=None, halign="center"))
            stack.add_widget(self.productDisplay(
                inv.getItemFromBarcode(self.barcode.text.strip())))
        elif self.nameField.text.strip() != "":
            stack.add_widget(MDLabel(
                text=f"[b]Name : {self.nameField.text.strip()}[/b]", markup=True, size_hint_y=None, halign="center"))
    ...
    def productDisplay(self, plist):
        columns = ["Name", "Barcode", "Department", "Cost Per Piece",
                   "Price", "Quantity", "Tax Category", "Deposit Category"]
        layout = MDGridLayout(cols=2, adaptive_height=True, row_force_default=True,
                              row_default_height=100, padding=[80, 25, 5, 35])
    ...
    def department(self, *args):
        ...
        self.chnageBack()
        self.layout = MDRelativeLayout(
            pos_hint={"x": 0, "y": 0.125}, size_hint=[1, 0.7],)
        if not self.sv is None:
            self.layout.remove_widget(self.sv)
            self.sv = None
        ...
        for i in inv.getCategory("Department_Category"):
            if i[3] in ["Default", "Miscellaneous/General", "Deli/General"]:
                stack.add_widget(
                    MDRaisedButton(size_hint=[0.9, 0.9], padding=[75, 100, 75, 100], anchor_x='left',
    ...
    def taxandDeposit(self, *args):
        ...
        self.chnageBack()
        self.layout = MDRelativeLayout(
            pos_hint={"x": 0, "y": 0.07}, size_hint=[1, 0.75],)
        if not self.sv is None:
            self.layout.remove_widget(self.sv)
            self.sv = None
        self.sv = MDRelativeLayout(
    ...
    def addCategory(self, widget, *args):
        ...
        if 'Department' in widget.text:
            ...
            self.layout.add_widget(
                MDLabel(text="[b]Category ID Number ;[/b]", pos_hint={"x": 0.1, "y": 0.65}, size_hint=[
                        0.35, 0.15], markup=True, font_size=fontsize['label'])
            )
            self.layout.add_widget(
                MDLabel(text="[b]Category Name ;[/b]", pos_hint={"x": 0.1, "y": 0.4}, size_hint=[
                        0.35, 0.15], markup=True, font_size=fontsize['label'])
            )
            self.layout.add_widget(
                MDLabel(text=f"[b]Deapartment_{inv.nextCategoryId('Department_Category')}[/b]", pos_hint={
                        "x": 0.45, "y": 0.65}, size_hint=[0.4, 0.15], markup=True, font_size=fontsize['label'])
            )
            self.nameField = MDTextField(font_size=fontsize['button'], on_text_validate=self.addCategorySubmit, helper_text="Department_"+str(
                inv.nextCategoryId('Department_Category')), hint_text="Enter Department Name..", pos_hint={"x": 0.45, "y": 0.4}, size_hint=[0.4, 0.15])
            ...
            self.layout.add_widget(
                MDLabel(text="[b]Category ID Number ;[/b]", pos_hint={"x": 0.1, "y": 0.8}, size_hint=[
                        0.35, 0.15], markup=True, font_size=fontsize['label'])
            )
            self.layout.add_widget(
                MDLabel(text=f"[b]Tax_{inv.nextCategoryId('Tax_Category')}[/b]", pos_hint={
                        "x": 0.45, "y": 0.8}, size_hint=[0.4, 0.15], markup=True, font_size=fontsize['label'])
            )
            self.layout.add_widget(
                MDLabel(text="[b]Category Name ;[/b]", pos_hint={"x": 0.1, "y": 0.6}, size_hint=[
                        0.35, 0.15], markup=True, font_size=fontsize['label'])
            )
            self.nameField = MDTextField(font_size=fontsize['button'], helper_text="Tax_"+str(inv.nextCategoryId(
                'Tax_Category')), hint_text="Enter Tax Category Name..", required=True, pos_hint={"x": 0.45, "y": 0.6}, size_hint=[0.4, 0.15])
            self.layout.add_widget(self.nameField
                                   )

            self.layout.add_widget(
                MDLabel(text="[b]Tax Value ;[/b]\nAdd '%' sign at end to act as percentage value, like 8.625%",
                        pos_hint={"x": 0.1, "y": 0.4}, size_hint=[0.35, 0.15], markup=True, font_size=fontsize['label'])
            )

            self.barcode = MDTextField(on_text_validate=self.validateValue, font_size=fontsize['button'], helper_text="Tax_"+str(
                inv.nextCategoryId('Tax_Category')), required=True, hint_text="Enter Tax Value", pos_hint={"x": 0.45, "y": 0.4}, size_hint=[0.4, 0.15])
            self.layout.add_widget(self.barcode
                                   )
            ...
            self.layout.add_widget(
                MDLabel(text=f"[b]Deposit_{inv.nextCategoryId('Deposit_Category')}[/b]", pos_hint={
                        "x": 0.45, "y": 0.8}, size_hint=[0.4, 0.15], markup=True, font_size=fontsize['label'])
            )
            self.layout.add_widget(
                MDLabel(text="[b]Category Name ;[/b]", pos_hint={"x": 0.1, "y": 0.6}, size_hint=[
            ...
            self.nameField = MDTextField(font_size=fontsize['button'], helper_text="Deposit_"+str(inv.nextCategoryId(
                'Deposit_Category')), hint_text="Enter Deposit Category Name..", required=True, pos_hint={"x": 0.45, "y": 0.6}, size_hint=[0.4, 0.15])
            self.layout.add_widget(self.nameField
                                   )

            self.layout.add_widget(
                MDLabel(text="[b]Deposit Value ;[/b]\nJust sumbit decimal amount\nlike 0.05 or 0.25", pos_hint={
                        "x": 0.1, "y": 0.4}, size_hint=[0.35, 0.15], markup=True, font_size=fontsize['label'])
            )

            self.barcode = MDTextField(on_text_validate=self.validateValue, font_size=fontsize['button'], helper_text="Deposit_"+str(inv.nextCategoryId(
                'Deposit_Category')), required=True, hint_text="Enter Deposit Value", pos_hint={"x": 0.45, "y": 0.4}, size_hint=[0.4, 0.15])
            self.layout.add_widget(self.barcode
                                   )
    ...
    def addCategorySubmit(self, widget, *args):
        if self.nameField.text.strip() == "":
            ...
            inv.addCategoryData(datetime.now(), 'Department_Category',
                                self.nameField.helper_text, self.nameField.text.strip(), None)
            if self.dialog:
                self.remove_widget(self.dialog)
            self.dialog = MDDialog(
            ...
            if self.validateValue():
                inv.addCategoryData(datetime.now(), 'Tax_Category', self.nameField.helper_text,
                                    self.nameField.text.strip(), self.barcode.text.strip())
                if self.dialog:
                    self.remove_widget(self.dialog)
                self.dialog = MDDialog(
            ...
            if self.validateValue():
                inv.addCategoryData(datetime.now(), 'Deposit_Category', self.nameField.helper_text,
                                    self.nameField.text.strip(), self.barcode.text.strip())
                if self.dialog:
                    self.remove_widget(self.dialog)
                self.dialog = MDDialog(
    ...
    def updateCategory(self, widget, *args):
        ...
        if 'Department' in widget.text:
            ...
            self.layout.add_widget(MDLabel(text="[b]Category ID Number ;[/b]", pos_hint={
                                   "x": 0.1, "y": 0.75}, size_hint=[0.35, 0.15], markup=True, font_size=fontsize['label']))
            self.layout.add_widget(MDLabel(text="[b]Category Name ;[/b]", pos_hint={
                                   "x": 0.1, "y": 0.58}, size_hint=[0.35, 0.15], markup=True, font_size=fontsize['label']))
            self.layout.add_widget(MDLabel(text="[b]Update Category Name ;[/b]", pos_hint={
                                   "x": 0.1, "y": 0.4}, size_hint=[0.35, 0.15], markup=True, font_size=fontsize['label']))

            self.layout.add_widget(
                MDLabel(text=widget.text.split("\n")[0], pos_hint={"x": 0.45, "y": 0.75}, size_hint=[
                        0.4, 0.15], markup=True, font_size=fontsize['label'])
            )
            self.layout.add_widget(
                MDLabel(text=widget.text.split("\n")[1], pos_hint={"x": 0.45, "y": 0.58}, size_hint=[
                        0.4, 0.15], markup=True, font_size=fontsize['label'])
            )
            ...
            self.layout.add_widget(MDLabel(text="[b]Category ID Number ;[/b]", pos_hint={
                                   "x": 0.1, "y": 0.8}, size_hint=[0.35, 0.15], markup=True, font_size=fontsize['label']))
            self.layout.add_widget(
                MDLabel(text=widget.text.split("\n")[0], pos_hint={"x": 0.45, "y": 0.8}, size_hint=[
                        0.4, 0.15], markup=True, font_size=fontsize['label'])
            )

            self.layout.add_widget(MDLabel(text="[b]Category Name ;[/b]", pos_hint={
                                   "x": 0.1, "y": 0.7}, size_hint=[0.35, 0.15], markup=True, font_size=fontsize['label']))
            self.layout.add_widget(
                MDLabel(text=widget.text.split("\n")[1], pos_hint={"x": 0.45, "y": 0.7}, size_hint=[
                        0.4, 0.15], markup=True, font_size=fontsize['label'])
            )

            self.layout.add_widget(MDLabel(text="[b]Tax Percentage ;[/b]", pos_hint={
                                   "x": 0.1, "y": 0.60}, size_hint=[0.35, 0.15], markup=True, font_size=fontsize['label']))
            self.layout.add_widget(
                MDLabel(text=widget.text.split("\n")[2], pos_hint={"x": 0.45, "y": 0.60}, size_hint=[
                        0.4, 0.15], markup=True, font_size=fontsize['label'])
            )

            self.layout.add_widget(MDLabel(text="[b]Update Category Name ;[/b]", pos_hint={
                                   "x": 0.1, "y": 0.45}, size_hint=[0.35, 0.15], markup=True, font_size=fontsize['label']))
            ...
            self.layout.add_widget(MDLabel(text="[b]Update Tax Percentage ;[/b]", pos_hint={
                                   "x": 0.1, "y": 0.315}, size_hint=[0.35, 0.15], markup=True, font_size=fontsize['label']))
            ...
            self.layout.add_widget(MDLabel(text="[b]Deposit Amount ;[/b]", pos_hint={
                                   "x": 0.1, "y": 0.60}, size_hint=[0.35, 0.15], markup=True, font_size=fontsize['label']))
            self.layout.add_widget(
                MDLabel(text=widget.text.split("\n")[2], pos_hint={"x": 0.45, "y": 0.60}, size_hint=[
                        0.4, 0.15], markup=True, font_size=fontsize['label'])
            ...
            self.layout.add_widget(MDLabel(text="[b]Update Deposit Amount ;[/b]", pos_hint={
                                   "x": 0.1, "y": 0.315}, size_hint=[0.35, 0.15], markup=True, font_size=fontsize['label']))
            self.barcode = MDTextField(font_size=fontsize['button'], helper_text=widget.text.split("\n")[
    ...
    def updateCategorySubmit(self, widget, *args):
        if ('Department' in widget.text) or ('Department' in self.nameField.helper_text):
            if self.nameField.text.strip() == "":
                ...
                self.dialog.open()
            inv.updateNameFromId(self.nameField.text.strip(),
                                 self.nameField.helper_text)
            if self.dialog:
                self.remove_widget(self.dialog)
            self.dialog = MDDialog(
        ...
        if ('Tax' in widget.text):
            if self.nameField.text.strip() == "" and self.barcode.text.strip() == "":
                ...
                if self.nameField.text.strip():
                    inv.updateNameFromId(
                        self.nameField.text.strip(), self.nameField.helper_text)
                    if self.dialog:
                        self.remove_widget(self.dialog)
                    self.dialog = MDDialog(
                ...
                if self.barcode.text.strip() and self.validateValue():
                    inv.updateValueFromId(
                        self.barcode.text.strip(), self.barcode.helper_text)
                    if self.dialog:
                        self.closeDialog()
                        self.remove_widget(self.dialog)
                    self.dialog = MDDialog(
