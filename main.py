...
class MainScreen(Screen):
    ...
    def on_pre_enter(self, *args):
        self.shiftLabel = MDLabel(text=f"Shift No: {shift['Shift No']}", md_bg_color=[
                                  0.128, 0.128, 0.128, 0.2], halign="center", font_style="H6", size_hint=[0.195, 0.13], pos_hint={"x": 0.1375, "y": 0.715})
        self.printButton = Button(text="[b]Print Recipt : ON[/b]" if printerSettings["Print_Reciept"] else "[b]Print Recipt : OFF[/b]", pos_hint={
                                  "x": 0.1375, "y": 0.5375}, markup=True, halign="center", background_color="lime" if printerSettings["Print_Reciept"] else "red", size_hint=[0.195, 0.135], on_press=self.printOnOff)
        self.cashDrawer = Button(text="[b]Cash Drawer\n\nCONNECTED[/b]" if printerSettings["Cash_Drawer"] else "[b]Cash Drawer\n\nNOT-CONNECTED[/b]", pos_hint={
                                 "x": 0.1375, "y": 0.35}, markup=True, halign="center", background_color="lime" if printerSettings["Cash_Drawer"] else "red", size_hint=[0.195, 0.15])
...
class loginShift(Screen):
    def startShift(self, *args):
        ...
        sf.addData(shift["Shift StartTime"], "",
                   shift["Shift No"], shift["Shift Names"], "")
